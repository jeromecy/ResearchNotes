...
ui <- navbarPage(
  ...
  title = "Three Level Drilldown Plot in RShiny",
  
  tabPanel("Plot",
           fluidPage(
             fluidRow(
               column(8, offset = 2,  # Center the plot and control its width
                      highchartOutput("drilldown_plot", height = "600px")
               )
             ),
             fluidRow(
               column(8, offset = 2,
                      p("Note: The data used in this plot is synthetic and not based on real data.")
               )
             )
           )
  ),
  ...
  tabPanel("About",
           fluidPage(
             h3("About This App"),
             p("This Shiny app demonstrates a three-level drilldown plot using Highcharter."),
             p("You can explore yield loss values by region, crop, and disease.")
           )
  ),
...
server <- function(input, output) {
  ...
  output$original_code <- renderUI({
    code <- readLines("SourceCode.R")  # Update with the path to your original code file
    code <- paste(code, collapse = "\n")
    HTML(paste0("<pre><code>", code, "</code></pre>"))
  })
  
  output$drilldown_plot <- renderHighchart({
    # Define colors
